...
# PURPOSE:  OVC care - combined plots
# LICENSE:  MIT
# DATE:     2021-08-13
...
filter(OVC_HIVSTAT > 0) %>% 
  ...
  scale_y_continuous(label = scales::comma,
                     expand = c(.005, .005)) +
  geom_text(aes(label = format(OVC_HIVSTAT, big.mark = ","), vjust = -1, na.rm = TRUE,
...
v3 <- df_art %>% 
  ...
  geom_line(size = 1.5, color = burnt_sienna) +
  geom_point(shape = 21, color = burnt_sienna, fill = "white", size = 10, stroke = 2) +
...
si_save("Graphics/OVC_GHLM2.svg")
