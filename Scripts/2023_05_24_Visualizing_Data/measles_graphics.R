...
# LOAD DATA ============================================================================  
    ...
    df_long <- 
      ...
      mutate(cases = ifelse(cases == 0.00, NA_real_, cases),
...
# TIME SERIES VIZ  ============================================================================
    ...
    ts_plot1 <- df_long %>% 
      ...
      labs(x = NULL, y = NULL, title = "MEASLES CASES PER 100K PEOPLE DRASTICALLY DECLINED STARTING IN THE MID 1960s",
    ...
    ts_plot1 +
      ...
      annotate("text", label="Vaccine introduced", x = 1963, y = 33e3, 
               vjust=1, hjust=0)
    ...
    ts_state_plot <- df_long %>% 
      ...
      scale_x_continuous(breaks=seq(1930, 2010, by=5), expand = c(0.025,0)) +
      scale_y_continuous(labels = label_number_si(), expand = c(0.025, 0)) +
      labs(x = NULL, y = NULL, title = "MEASLES CASES 100K PEOPLE DRASTICALLY DECLINED STARTING IN THE MID 1960s",
           caption = "Source: Weekly Measles Incidence, 1928-2003 | Project Tycho",
    ...
    ts_state_plot2 +
      geom_vline(xintercept = 1963, linewidth = 1, color = grey90k) +
      annotate("text", label="Vaccine introduced", x = 1963, y = 3e3, 
               vjust=1, hjust=0)
...
# HEATMAP ============================================================================
    ...
    cols2 <- c(colorRampPalette(c("white", grey10k, grey20k, grey30k, grey40k, grey50k))(10),
               colorRampPalette(c(grey50k, grey60k, grey70k, grey80k, grey90k, "black"), bias=2)(90))
    
    prpl <- c(colorRampPalette(c("white", "#E9DDFF", "#CFC3FF", "#B5AAF9", "#9E94E0", "#877EC9"))(10),
    ...
    plot_a +
      scale_fill_gradientn(colours = prpl,
                           na.value = "#f1f3e2",
                           #na.value = "#f9ebed", 
                           #na.value = "white",
      ...
      annotate("text", label="Endemic Measles Eliminated", x = 2000, y=53, 
               vjust=1, hjust=0, size=I(3)) 
    
    
    si_save("../catch-22/AI/measles_heatmap_base_clr_annotate2.svg")
