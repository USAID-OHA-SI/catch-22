...
# PURPOSE:  Dec 2021 call with IPs/OUs - TX_CURR trends
# LICENSE:  MIT
# DATE:     2021-12-07
# NOTE: catch-22/Scripts/20211102_Local Partner Meeting/20211022_LP_TX_trends.R
...
df_tx_curr <- df_tx %>% 
  filter(partner_type != "International",
         indicator == "TX_CURR") %>% 
  mutate(ind_label = case_when(partner_type == "Local" ~ "Local Partner contribution",
                               TRUE ~ "Total currently receiving antiretroviral therapy"))
...
df_tx_curr %>% 
  ...
  geom_text(aes(label = percent(share, 1)), vjust = -1, 
            size = 10/.pt, 
            label.size = NA, family = "Source Sans Pro SemiBold") +
  scale_fill_manual(values = c(genoa, genoa_light)) +
  scale_alpha_identity() +
  labs(x = NULL, y = NULL, fill = NULL,
       title = glue("LOCAL PARTNERS ARE POISED TO ADD MORE PEOPLE ONTO TREATMENT THIS YEAR THAN PREVIOUS YEAR,
       ALREADY REACHING {title_info_tx_curr$value} PATIENTS AND {title_info_tx_curr$share} OF USAID'S TREATMENT PORTFOLIO"),
