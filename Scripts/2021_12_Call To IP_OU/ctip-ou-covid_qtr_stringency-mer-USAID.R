...
# PURPOSE:  Call with IPs/OUs - COVID Stringency Index + MER Trends for USAID ONLY
# LICENSE:  MIT
# DATE:     2021-12-06
# UPDATED:  2021-12-08
# NOTES:    adapted from agitprop/17b_stringency_mer.R and agitprop/Scripts/33_covid_qtr_stringency-mer.R
...
#select indicator and reshape long for USAID ONLY (comment out filter if you want PEPFAR)
df_mer <- df %>% 
  filter(indicator %in% c("HTS_TST", "HTS_TST_POS","TX_NEW", "TX_CURR", "TX_PVLS", "VMMC_CIRC"),
         ...
         fiscal_year >= 2019,
         mech_code != "16772", 
         fundingagency == "USAID") %>% #comment out for all of PEPFAR 
...
plot_mer_stringency <- function(ind_sel, n_countries = 16, save = FALSE){
  ...
  v <- df_viz %>% 
    ...
    labs(x = NULL, y = NULL,
         title = "USAID's work has remained resilient in the face of government policies to curb COVID" %>% toupper,
  ...
  if(save == TRUE){
    file_out <- glue("Graphics/ctip-cwou_country_stringency_{ind_sel}_USAID.svg")
