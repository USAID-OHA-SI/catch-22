...
# DATE:     2021-11-09
# UPDATED:  2021-11-12
# NOTE:     based on groundhogday/FY21Q3-Review_MMD-trends.R
...
# VIZ INFO ----------------------------------------------------------------
  ...
  #asia region
  xar_cntry <- df_mmd_ou %>% 
    filter(operatingunit %in% c("Asia Region", "Vietnam","USAID"),
           otherdisaggregate == "o6mmd",
           period == max(period)) %>% 
    pull(countryname)
...
# VIZ ---------------------------------------------------------------------
  ...
  si_save("Graphics/gpm_usaid_mmd-trends-by-country.svg", height = 4.25)    
  
  
  df_mmd_ou %>%
    filter(countryname %in% xar_cntry) %>%
    ggplot(aes(period, share, group = otherdisaggregate, color = fill_color, fill = fill_color)) +
    geom_area(alpha = .4, size = .9, position = "identity") +
    geom_point(aes(y = endpoints), na.rm = TRUE) +
    facet_wrap(~fct_reorder2(country_lab, period, tx_curr, .desc = TRUE)) +
    ...
    expand_limits(y = 1) +
    labs(x = NULL, y = NULL,
         title = glue("USAID HAS MADE LIMITED GAINS TOWARDS GETTING TREATMENT PATIENTS ON +6 MONTHS OF MMD SINCE FY20Q3 ESPECIALLY IN ASIA REGION"),
         subtitle = "South Africa, representing a third of USAID's treatment portfolio, has been excluded from USAID total",
         caption = glue("MMD 3 months or more = 3-5 months and 6 months or more",  
                        "Source: {msd_source}",
                        "USAID SI Analytics",
                        "Global Planning Meeting 2021-11-15", .sep = " | ")) +
  ...
  si_save("Graphics/gpm_usaid_mmd-trends-by-country_asia.svg", height = 4.25) 
