...
# IMPORT ------------------------------------------------------------------
  ...
  #limit date to match other deck
    df_covid <- filter(df_covid, date <= "2021-10-26")
...
#           height = 4.25)  
  ...
  df_reg_agg <- df_covid %>% 
    left_join(pepfar_country_list %>% 
                select(countryname, operatingunit)) %>% 
    mutate(region = ifelse(operatingunit %in% c("Asia Region", "Vietnam"), "Asia Region + Vietnam", "Rest of PEPFAR")) %>% 
  ...
  v2 <- df_reg_agg %>% 
    ...
    scale_x_date(date_labels = "%b '%y",
                breaks = qtrs) +
    scale_fill_si(palette = "moody_blue", alpha = .8, discrete = TRUE) +
  ...
  v1 + v2 + #plot_layout(widths = c(2, 1)) +
    plot_annotation(title = "COVID HAS INTRODUCED PROGRAM AND REPORTING HURDLES FOR PEPFAR",
                    # caption = glue("Source: Source: JHU COVID-19 feed [{today()}]",
                    caption = glue("Source: Source: JHU COVID-19 feed [2021-11-04]",
                                   "USAID SI Analytics",
                                   "Global Planning Meeting 2021-11-15", .sep = " | "),
                    theme = si_style_ygrid())
  
  
  si_save("Graphics/gpm_historic_pepfar-covid-regional-share-asia.svg",
          height = 4.25)  
