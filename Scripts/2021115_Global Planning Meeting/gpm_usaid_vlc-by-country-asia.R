...
# PURPOSE:  Explore VLS aross USAID mechanisms for Asia Region
# LICENSE:  MIT
# DATE:     2021-10-18
# UPDATED:  2021-11-12
# NOTE:     derived from catch-22/20211018_USAID_VLC-by-country.R
...
# MUNGE SITE ADJUSTMENT ---------------------------------------------------
  ...
  df_vls_adj <- df_vls_adj %>% 
    ...
    group_by(operatingunit, countryname, type, mech_code, mech_name, primepartner) %>% 
...
# MUNGE SITE ADJUSTMENT ---------------------------------------------------
  ...
  df_viz_vlc_asia <- df_viz_vlc %>% 
    filter(operatingunit %in% c("Asia Region", "Vietnam"))
  ...
  df_viz_vlc_asia %>% 
    ...
    scale_size(labels = number_format(1, scale = 1e-3, suffix = "K"),
               range = c(2,10)) +
  ...
  si_save("Graphics/gpm_usaid_vlc_asia.svg",
          height = 4.25)
